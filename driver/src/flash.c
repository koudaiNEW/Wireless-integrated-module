...
en_result_t Flash_Init(uint8_t u8FreqCfg, boolean_t bDpstbEn)
{
    ...
    {
        user_u32PrgTimer[u32Index] = u8FreqCfg * pu32PcgTimer4M[u32Index];
    } 
    ...
    for(u32Index=0; u32Index<8; u32Index++)
    {
        ...
        while(pu32PrgTimerReg[u32Index]  != user_u32PrgTimer[u32Index])
        {
            if(u32TimeOut--)
            {
                FLASH_BYPASS();
                pu32PrgTimerReg[u32Index] = user_u32PrgTimer[u32Index];
            }
            else
            {
